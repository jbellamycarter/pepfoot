...
from gui.pepfoot import Ui_MainWindow
...
class Main(Qtw.QMainWindow, Ui_MainWindow):
    ...
    def export_csv(self):
        ...
        with open(file_name, 'w') as outfile:
            ...
            if treatments:
                header += 'Mean F.mod 1,S.dev. F.mod 1,Mean F.mod 2,S.dev. F.mod 2,p value,'
            else:
            ...
            for i, pep in enumerate(self.project['peptides']):
                ...
                if treatments:
                    line += '{:.4f},'.format(fmod[treatments[0]].mean(axis=0)[i])
                    line += '{:.4f},'.format(fmod[treatments[0]].std(axis=0)[i])
                    line += '{:.4f},'.format(fmod[treatments[1]].mean(axis=0)[i])
                    line += '{:.4f},'.format(fmod[treatments[1]].std(axis=0)[i])
                    line += '{:.2f},'.format(p_values[i])

                else:
                    line += '{:.4f},'.format(fmod.mean(axis=0)[i])
                    line += '{:.4f},'.format(fmod.std(axis=0)[i])
    ...
    def update_analysis(self):
        ...
        try:
            ...
            if treatment:
                ...
                pvalue = stats.ttest_ind(fmod[treatment[0]], fmod[treatment[1]], axis=0)[1]
                ...
                if not self.DiffPlotChk.isChecked():
                    ...
                    diff_stdev = np.sqrt(stdev1**2 + stdev2**2)
                    mask3 = diff_mean != 0

                    extent_mean = np.zeros_like(diff_mean)
                    extent_mean[mask3] = diff_mean[mask3] / mean1[mask3]
                    extent_stdev = np.zeros_like(diff_stdev)
                    extent_stdev[mask3] = extent_mean[mask3] * np.sqrt((diff_stdev[mask3]/diff_mean[mask3])**2 + (stdev1[mask3]/mean1[mask3])**2)
                    mask4 = extent_mean > 0

                    self.barAx1.bar(idx[mask4], extent_mean[mask4], 0.4, yerr=extent_stdev[mask4], color=GREEN, error_kw={'elinewidth':1}, capsize=2)
                    self.barAx1.bar(idx[~mask4], extent_mean[~mask4], 0.4, yerr=extent_stdev[~mask4], color=REDPURPLE, error_kw={'elinewidth':1}, capsize=2)
                    ...
                    self.barAx2.bar(pep_ids[:,0][mask4], heights[mask4], widths[mask4], bottoms[mask4], align='edge', color=get_colours(GREEN, extent_mean[mask4]/extent_max, threshold))
                    self.barAx2.bar(pep_ids[:,0][~mask4], heights[~mask4], widths[~mask4], bottoms[~mask4], align='edge', color=get_colours(REDPURPLE, -extent_mean[~mask4]/extent_max, threshold))
                    self.barAx2.axhline(0, lw=1, c='0.5')
            ...
            if self.PDB_LOADED:
                self.update_bfactor(np.array(self.project['peptides']), sig_mask, sign_mask)
    ...
    def show_bar_info(self, idx):
        ...
        if self.project['treatment']:
            ...
            _text += '    Mean: {:.2f}    '.format(fmod[treatments[0]].mean(axis=0)[idx])
            _text += 'Deviation: {:.2f}\n'.format(fmod[treatments[0]].std(axis=0)[idx])
            _text += 'Holo\n'
            for j in treatments[1]:
                _text += '{:.2f}    '.format(fmod[j][idx])
            _text += '    Mean: {:.2f}    '.format(fmod[treatments[1]].mean(axis=0)[idx])
            _text += 'Deviation: {:.2f}\n'.format(fmod[treatments[1]].std(axis=0)[idx])

        else:
            for i in range(len(fmod)):
                _text += '{:.2f}    '.format(fmod[i][idx])
            _text += '    Mean: {:.2f}    '.format(fmod.mean(axis=0)[idx])
            _text += 'Deviation: {:.2f}\n'.format(fmod.std(axis=0)[idx])
    ...
    def update_bfactor(self, ids, sig_mask, sign_mask):
        """Calculate B-factors from fmod data and apply to self.pdb

        Not detected    -1  White/LightGrey
        Insignificant    0  Wheat/Yellow
        Significant      1  Orange
        Significant -    2  Blue
        ...
        for i, peptide in enumerate(ids[sig_mask]):
            if sign_mask[i] == 1:
                bfactors[peptide[0]:peptide[1]] = 1
            else:
                bfactors[peptide[0]:peptide[1]] = 2
      ...
      var myBfactor = NGL.ColormakerRegistry.addScheme(function (params) {
          this.atomColor = function (atom) {
            if (atom.bfactor == 0) {
              ...
              return 0xD55E00; //vermillion
            } else if (atom.bfactor == 2) {
              return 0x0072B2; // blue
            } else {
        ...
        stage.setParameters({backgroundColor: 0xFF0000});
        protein.autoView();
